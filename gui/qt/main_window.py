...
from tate.i18n import _, set_language
...
from tate.bitcoin import MIN_RELAY_TX_FEE, is_valid
...
from tate import util, bitcoin, commands, Interface, Wallet
...
class ElectrumWindow(QMainWindow):
    ...
    def load_wallet(self, wallet):
        ...
        title = 'Tate ' + self.wallet.electrum_version + '  -  ' + self.wallet.storage.path
    ...
    def create_send_tab(self):
        ...
        self.payto_help = HelpButton(_('Recipient of the funds.') + '\n\n' + _('You may enter a Mazacoin address, a label from your list of contacts (a list of completions will be proposed), or an alias (email-like address that forwards to a Mazacoin address)'))
    ...
    def read_send_tab(self):
        ...
        for type, addr, amount in outputs:
            if addr is None:
                QMessageBox.warning(self, _('Error'), _('Mazacoin Address is None'), _('OK'))
                return
            if type == 'op_return':
                continue
            if type == 'address' and not bitcoin.is_address(addr):
                QMessageBox.warning(self, _('Error'), _('Invalid Mazacoin Address'), _('OK'))
                return
            if amount is None:
    ...
    def create_contact_menu(self, position):
        ...
        if not item:
            ...
            menu.addAction(_("QR code"), lambda: self.show_qrcode("mazacoin:" + addr, _("Address")))
            if is_editable:
    ...
    def new_account_dialog(self, password):
        ...
        e = QLineEdit()
        vbox.addWidget(e)
        msg = _("Note: Newly created accounts are 'pending' until they receive mazacoins.") + " " \
    ...
    def settings_dialog(self):
        ...
        d = QDialog(self)
        d.setWindowTitle(_('Tate Settings'))
        d.setModal(1)
        vbox = QVBoxLayout()
        grid = QGridLayout()
        ...
        block_ex_combo.setCurrentIndex(block_explorers.index(self.config.get('block_explorer', 'Mazacha.in')))
    ...
    def plugins_dialog(self):
        from tate.plugins import plugins
